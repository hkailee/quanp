...
class QuanpyConfig:
    ...
    def autosave(self) -> bool:
        """\
        Automatically save figures in :attr:`~quanp._settings.QuanpyConfig.figdir` (default `False`).
    ...
    def writedir(self) -> Path:
        """\
        Directory where the function quanp.write writes to by default.
        """
    ...
    def datasetdir(self) -> Path:
        """\
        Directory for example :mod:`~quanp.datasets` (default `'./data/'`).
        """
    ...
    def logdir(self) -> Path:
        """\
        Directory for example :mod:`~quanp.datasets` (default `'./log/'`).
        """
        return self._logdir

    @logdir.setter
    def logdir(self, logdir: Union[str, Path]):
        _type_check(logdir, "logdir", (str, Path))
        self._logdir = Path(logdir).resolve()
    ...
    def set_figure_params(
        ...
        ----------
        quanp
            Init default values for :obj:`matplotlib.rcParams` suited for Quanpy.
        dpi
        ...
        fontsize
            Set the fontsize for several `rcParams` entries. Ignored if `quanp=False`.
        figsize
            Set plt.rcParams['figure.figsize'].
        color_map
            Convenience method for setting the default color map. Ignored if `quanp=False`.
        format
        ...
        if facecolor is not None:
            ...
            rcParams['axes.facecolor'] = facecolor
        if quanp:
            from .plotting._rcmod import set_rcParams_quanp
            set_rcParams_quanp(fontsize=fontsize, color_map=color_map)
