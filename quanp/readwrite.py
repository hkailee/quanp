...
def read(
    ...
    **kwargs) -> AnnData:
    ...
    if not filename.exists():
        raise ValueError(
            'Reading with filekey {} failed, \
            the inferred filename {} does not exist. \
            If you intended to provide a filename, either use a filename \
            ending on one of the available extensions {} \
            or pass the parameter `ext`.'.format(filekey, filename, avail_exts)
        )
...
#     gex_only: bool = True,
#     backup_url: Optional[str] = None) -> AnnData:
#     """\
...
def read_params(
    filename: Union[Path, str], asheader: bool = False) -> Dict[str, Union[int, float, bool, str, None]]:
    """\
...
def _read(
    ...
    **kwargs):
    ...
    is_present = _check_datafile_present_and_download(filename, backup_url=backup_url,)
    if not is_present:
        logg.debug('... did not find original file {}'.format(filename))
    # read hdf5 files
    if ext in {'h5', 'h5ad'}:
        if sheet is None:
            return read_h5ad(filename, backed=backed)
        else:
            logg.debug('reading sheet {} from file {}'.format(sheet, filename))
    ...
    if cache and path_cache.is_file():
        logg.info('... reading from cache file {}'.format(path_cache))
        return read_h5ad(path_cache)

    if not is_present:
        raise FileNotFoundError('Did not find file {}.'.format(filename))
    logg.debug('reading {}'.format(filename))
    ...
    if cache:
        logg.info(
            '... writing an {} cache file to speedup \
                reading next time'.format(settings.file_format_data)
        )
...
def _get_filename_from_key(key, ext=None) -> Path:
    ext = settings.file_format_data if ext is None else ext
    return settings.writedir / '{key}.{ext}'.format(key, ext)
...
def _check_datafile_present_and_download(path, backup_url=None):
    ...
    logg.info(
        ...
        ... this may take a while but only happens once'.format(backup_url)
    )
    if not path.parent.is_dir():
        logg.info('creating directory {}/ for saving data'.format(path.parent))
        path.parent.mkdir(parents=True)
...
def is_valid_filename(filename: Path, return_ext=False):
    ...
    if len(ext) > 2:
        logg.warning(
            'Your filename has more than two extensions: {ext}.\n \
